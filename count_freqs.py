    ...
    def train(self, corpus_file):
        ...
        for ngram in ngram_iterator:
            ...
            if ngram[-1][0] is not None: # If this is not the last word in a sentence
                ...
                if ngram[-1][1] not in self.tag_counts:
                    self.tag_counts[ngram[-1][1]] = 1
                else:
                    self.tag_counts[ngram[-1][1]] += 1
        ...
        #assign _RARE_ class
        copy_emission_counts = self.emission_counts.copy()

        for k,v in copy_emission_counts.items():
            word = k[0]
            if word not in self.word_counts:
                self.word_counts[word] = v
            else:
                self.word_counts[word] += v
        ...
        for k,v in copy_emission_counts.items():
            ...
            if self.word_counts[word] < 5:
                if ('_RARE_', tag) not in self.emission_counts:
                    self.emission_counts[('_RARE_', tag)] = v
                else:
                    self.emission_counts[('_RARE_', tag)] += v

                self.emission_counts.pop(k)

        self.compute_emission_params()



    def compute_emission_params(self):
        for emKey,emVal in self.emission_counts.items():
            self.emission_probs[emKey] = emVal / self.tag_counts[emKey[1]]
